...
2. **Sends it to GPT-4o** with a bilingual (English/Japanese) extraction prompt.
...
└──────────────────────┘
        ...
        ▼
┌──────────────────────────────────┐
│  GPT-4o extraction (HTTP POST)   │  → JSON with invoice fields + confidence
└──────────────────────────────────┘
        │
        ▼
┌──────────────────────────────────┐
...
| OpenAI API key | GPT-4o access required (Tier 1 is sufficient for low volume) |
...
5. In **n8n → Variables**, create a variable named `EXTRACTION_PROMPT` and paste the full contents of `prompts/invoice_extraction.txt` as its value.
...
| What to change | Which node to edit |
|---|---|
...
### Notes on the OpenAI node choice
...
If your invoices arrive exclusively as images (PNG/JPG), you can swap the HTTP Request node for the native **OpenAI node** (operation: "Message a Model") and configure it with image input — this simplifies credential management.
...
4. ワークフロー内の `REPLACE_WITH_YOUR_*` の箇所を実際のIDに置き換えます：
   ...
   - SlackノードのチャンネルID
5. **n8n → 変数（Variables）** に `EXTRACTION_PROMPT` という名前の変数を作成し、`prompts/invoice_extraction.txt` の内容を貼り付けます。
